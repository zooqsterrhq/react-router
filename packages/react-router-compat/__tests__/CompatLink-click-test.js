...
import { Router, Link } from "react-router-compat";
...
describe("<Link> click events", () => {
  ...
  beforeEach(() => {
    ...
    replaceSpy = jest.spyOn(memoryHistory, "replace");
  });
  ...
  it("calls onClick eventhandler and history.push", () => {
    ...
    expect(pushSpy).toBeCalledWith(
      { hash: "#the-hash", pathname: "/the/path", search: "?the=query" },
      undefined
    );
  });
  ...
  it("calls history.replace on duplicate navigation", () => {
    ...
    expect(pushSpy).toBeCalledTimes(1);
    expect(pushSpy).toBeCalledWith(
      { hash: "#the-hash", pathname: "/duplicate/path", search: "?the=query" },
      undefined
    );
