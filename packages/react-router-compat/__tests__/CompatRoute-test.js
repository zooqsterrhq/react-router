...
import { MemoryRouter, Router, Route } from "react-router-compat";
...
describe("A <Route>", () => {
  ...
  describe("with an array of paths", () => {
    ...
    xit("matches other provided paths", () => {
      const node = document.createElement("div");
      ReactDOM.render(
    ...
    xit("provides the matched path as a string", () => {
      const node = document.createElement("div");
      ReactDOM.render(
        <MemoryRouter initialEntries={["/other", "/world"]} initialIndex={1}>
          <Route
    ...
    xit("doesn't remount when moving from one matching path to another", () => {
      const node = document.createElement("div");
      const history = createHistory();
      const mount = jest.fn();
  ...
  describe("the `component` prop", () => {
    ...
    xit("won't throw a prop-type warning when passed valid React components that aren't functions", () => {
